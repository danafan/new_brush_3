<template>
	<div class="container">
		...
		<div class="content" v-if="this.task_info.extra_goods_num > 0">
			<div class="content_title">
				<div class="tag_img"></div>
				<div class="title_text">额外任务要求</div>
			</div>
			<div class="content_box requirements">{{task_info.extra_goods_remark}}</div>
		</div>
...
<script>
	...
	export default{
		...
		methods:{
			...
			getTaskDetail(){
				resource.taskDetail({usertask_id:this.task_id}).then(res => {
					if(res.data.code == 1){
						...
						if(this.task_info.extra_goods_num > 0){
							MessageBox({
								$type:'alert',
								message:'本次任务由多任务组成，需要全部完成，详情可看任务要求!',
								title:'温馨提示',
			...
			submitTask(){
				if(this.task_info.extra_goods_num > 0){
					MessageBox.confirm(this.goods_price).then(action => {
						if(action == 'confirm'){
